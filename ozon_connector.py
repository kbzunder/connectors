...
## returns dataframes ready to bigquery 
    ...
    def get_data(date, json_1, json_2,headers,url_costs, url_comp):
        req_1 = requests.post(url_costs, headers=headers, json=json_1)
        if req_1.status_code == 200:
            j_1 = req_1.json()
            df_cost =    pd.DataFrame.from_dict(j_1) 
        ...
        date = start_date + 'T00:00:00.000Z'
        req_2 = requests.post(url_comp, headers=headers, json=json_2)
        if req_1.status_code == 200:
            j_2 = req_2.json()
...
## if input parameters are dates
 
    def date_range_string(start, end):
        start  = datetime.strptime(start, '%Y-%m-%d').date()
        end = datetime.strptime(end, '%Y-%m-%d').date()
        delta = end - start  # as timedelta
        days_st = [(start + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(delta.days + 1)]
        return days_st  

## transers data to bigquery tables
    
    def data_to_gbq(start, end, params):

        for day in date_range_string(start,end):
            
            df1 = get_data(params)[0]
            df2 = det_data(params)[1]
